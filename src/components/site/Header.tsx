...
export function Header() {
  ...
  const navGroupClass = "grid gap-1 rounded-[1.35rem] border border-white/45 bg-white/35 p-1 shadow-sm backdrop-blur-2xl backdrop-saturate-150 md:flex md:items-center md:rounded-full";
  const primaryNavGroupClass = "grid gap-1 rounded-[1.35rem] border border-primary/15 bg-primary/10 p-1 shadow-sm backdrop-blur-2xl backdrop-saturate-150 md:flex md:items-center md:rounded-full";
  const navLinkClass = "rounded-2xl px-4 py-3 text-base font-bold text-muted-foreground transition hover:bg-white/55 hover:text-primary hover:shadow-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/35 md:rounded-full md:px-4 md:py-2 md:text-sm";
  const activeNavClass = "bg-white/75 text-primary shadow-sm ring-1 ring-white/60 backdrop-blur-xl";
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-white/35 bg-white/45 shadow-sm shadow-primary/5 backdrop-blur-2xl backdrop-saturate-150 supports-[backdrop-filter]:bg-white/35">
      <div className="container mx-auto flex h-14 items-center justify-between gap-2 px-3 sm:h-16 sm:px-4">
        ...
        <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
          ...
          <Button
            ...
            variant="outline"
            size="sm"
            className="relative h-9 rounded-full border-primary/25 bg-primary/10 px-3 text-primary shadow-sm hover:bg-primary hover:text-primary-foreground"
          >
          ...
          {isAuthenticated ? (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56 rounded-2xl border-white/50 bg-white/70 p-1 shadow-xl backdrop-blur-2xl">
      ...
      {open && (
        <div className="container mx-auto border-t border-white/35 px-3 py-3 md:hidden">
          {isAuthenticated && <div className="mb-3 rounded-2xl border border-white/45 bg-white/45 px-4 py-3 text-sm font-semibold shadow-sm backdrop-blur-xl">Signed in as @{username}</div>}
          <nav className="grid gap-3">{nav}</nav>
          {isAuthenticated ? (
            <div className="mt-3 grid gap-2 border-t border-white/35 pt-3">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 border-t border-white/35 pt-3">
