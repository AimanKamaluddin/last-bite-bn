...
function Dashboard() {
  ...
  const cancel = async (id: string) => {
    ...
    if (!order || order.status !== "reserved") return toast.error("Only reserved orders can be cancelled.");
    const quantityToRestore = Math.max(0, Number(order.quantity ?? 0));
    ...
    <SiteLayout>
      <section className="container mx-auto max-w-5xl px-4 py-10">
        ...
        <Tabs defaultValue="upcoming" className="mt-6">
          <TabsList><TabsTrigger value="upcoming">Upcoming pickups</TabsTrigger><TabsTrigger value="notifications">Notifications</TabsTrigger><TabsTrigger value="past">Past orders</TabsTrigger><TabsTrigger value="saved">Saved merchants</TabsTrigger></TabsList>
...
function OrderRow({ o, onCancel, review, onReviewSaved, userId }: { o: any; onCancel?: () => void; review?: any; onReviewSaved?: (r: any) => void; userId?: string }) {
  ...
  return (
    <Card className="rounded-3xl p-4">
      ...
      {o.status === "collected" && userId && <ReviewForm order={o} existing={review} userId={userId} onSaved={onReviewSaved} />}
    </Card>
  );
}
