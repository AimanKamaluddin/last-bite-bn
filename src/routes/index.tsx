...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Brunei's surplus food marketplace. Reserve discounted meals from local bakeries, cafes, restaurants and supermarkets before closing." },
...
function Landing() {
  ...
  return (
    <SiteLayout>
      <section className="relative overflow-hidden">
        <div className="container mx-auto grid items-center gap-10 px-4 pb-12 pt-10 md:grid-cols-2 md:pb-20 md:pt-16">
          <div className="space-y-6">
            ...
            <p className="max-w-lg text-lg text-muted-foreground">Discover surprise bags and surplus meals from local bakeries, cafes, restaurants, hotels and supermarkets before they close.</p>
            <div className="flex flex-wrap gap-3"><Button asChild size="lg" className="rounded-full"><Link to="/browse"><Search className="mr-2 h-4 w-4" /> Browse Food</Link></Button><Button asChild size="lg" variant="outline" className="rounded-full"><Link to="/merchant/onboarding"><Store className="mr-2 h-4 w-4" /> Join as Merchant</Link></Button></div>
            <div className="grid gap-3 pt-2 text-sm text-muted-foreground sm:grid-cols-3"><TrustPoint title="Support local" body="Buy from Brunei food businesses." /><TrustPoint title="Reduce waste" body="Help rescue good food." /><TrustPoint title="Simple pickup" body="Reserve and collect in-store." /></div>
          </div>
          <div className="relative"><img src={heroImg} alt="Fresh pastries from a local bakery" width={1536} height={1024} className="aspect-[4/3] w-full rounded-3xl object-cover shadow-xl" /><Card className="absolute -bottom-4 -left-4 hidden w-64 rounded-2xl p-4 shadow-lg sm:block"><div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl bg-primary/10 text-primary"><Leaf className="h-5 w-5" /></div><div><div className="text-sm font-semibold">Every rescued meal helps</div><div className="text-xs text-muted-foreground">Support local businesses and reduce waste.</div></div></div></Card></div>
        </div>
      </section>
      ...
      {merchants.length > 0 && <Section title="Featured merchants" subtitle="Discover local food businesses on Last Bite."><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{merchants.map((m) => <Card key={m.id} className="overflow-hidden rounded-3xl p-0">{m.image_url ? <img src={m.image_url} alt={m.business_name} loading="lazy" className="h-36 w-full object-cover" /> : <div className="h-36 w-full bg-muted" />}<div className="p-4"><div className="font-semibold">{m.business_name}</div><div className="text-sm text-muted-foreground">{m.business_type} · {m.district}</div></div></Card>)}</div></Section>}
