...
function Landing() {
  ...
  return <SiteLayout>
      ...
      <div className="container relative mx-auto grid items-center gap-8 px-3 pb-10 pt-8 sm:px-4 md:grid-cols-[1.02fr_0.98fr] md:gap-12 md:pb-16 md:pt-14">
        ...
        <div className="relative min-h-[420px] md:min-h-[560px]">
          <div className="absolute left-0 top-8 z-10 rounded-3xl border border-white/70 bg-white/80 p-4 shadow-2xl shadow-primary/10 backdrop-blur md:left-2">
            <div className="flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-2xl bg-accent text-accent-foreground"><Flame className="h-5 w-5" /></div><div><div className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Today only</div><div className="text-lg font-black">Up to 60% off</div></div></div>
          </div>
          <Link to={heroListing ? "/listing/$id" : "/browse"} params={heroListing ? { id: heroListing.id } : undefined as any} className="group absolute inset-x-4 top-0 block overflow-hidden rounded-[2rem] border border-white/70 bg-white shadow-[0_35px_100px_-40px_hsl(var(--primary)/0.95)] transition hover:-translate-y-1 hover:shadow-[0_45px_120px_-45px_hsl(var(--primary)/1)] md:inset-x-12 md:rounded-[2.5rem]">
            <div className="relative h-[390px] overflow-hidden md:h-[520px]">
              ...
              <div className="absolute left-5 right-5 top-5 flex items-center justify-between"><Badge className="rounded-full bg-white px-4 py-1.5 text-primary shadow-md">Chef's rescue pick</Badge><Badge className="rounded-full bg-black/25 px-4 py-1.5 text-white backdrop-blur ring-1 ring-white/25"><Clock className="mr-1 h-3.5 w-3.5" /> {heroListing ? urgencyLabel(heroListing) : "Available today"}</Badge></div>
              <div className="absolute inset-x-0 bottom-0 p-5 text-white sm:p-7">
                <div className="mb-3 flex flex-wrap gap-2"><span className="rounded-full bg-white/15 px-3 py-1 text-xs font-semibold backdrop-blur">Brunei local</span><span className="rounded-full bg-white/15 px-3 py-1 text-xs font-semibold backdrop-blur">Fresh pickup</span></div>
                <h2 className="text-3xl font-black leading-tight sm:text-5xl">{heroListing?.title ?? "Fresh surprise bags"}</h2>
                <div className="mt-4 flex items-end justify-between gap-4">
                  <div>{heroListing ? <><div className="text-sm text-white/70 line-through">{formatBND(heroListing.original_price)}</div><div className="text-4xl font-black text-white">{formatBND(heroListing.discounted_price)}</div></> : <div className="text-3xl font-black">From BND 3</div>}<div className="mt-1 text-sm text-white/75">{heroListing?.merchant.business_name ?? "Local cafés, bakeries and restaurants"}</div></div>
          ...
          <div className="absolute bottom-2 right-0 rounded-3xl border border-white/70 bg-white/85 p-4 shadow-2xl shadow-accent/10 backdrop-blur md:right-4">
            ...
            <div className="text-xs text-muted-foreground">rescue deals near you</div>
          </div>
        </div>
      </div>
    </section>
    ...
    {availableNow.length > 0 && <Section eyebrow="Fresh drops" title="Today's rescue deals" notice={<PickupWindowAlert compact />} action={<Button asChild variant="outline" className="rounded-full bg-white/70"><Link to="/browse">{t("browseAll")} <ArrowRight className="h-4 w-4" /></Link></Button>}>
