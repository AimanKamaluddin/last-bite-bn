  ...
  useEffect(() => {
    ...
    (async () => {
      const [{ data: m }, { data: l }, { data: o }] = await Promise.all([
        ...
        (supabase as any).from("listings").select("*, merchants_public(business_name)").order("created_at", { ascending: false }),
        (supabase as any).from("orders").select("*, merchants_public(business_name), listings(title)").order("created_at", { ascending: false }),
      ]);
